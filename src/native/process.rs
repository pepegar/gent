use janetrs::{client::JanetClient, env::CFunOptions, Janet, JanetKeyword, JanetString, JanetTable};

/// (process/exec cmd &opt args)
/// Execute a command synchronously and return a table with :stdout, :stderr, :status.
/// For async/non-blocking execution, use os/spawn from Janet instead.
...
fn exec(args: &mut [Janet]) -> Janet {
    ...
    match output {
        Ok(out) => {
            ...
            result.insert(JanetKeyword::new("stdout"), JanetString::new(stdout.as_bytes()));
            result.insert(JanetKeyword::new("stderr"), JanetString::new(stderr.as_bytes()));
        ...
        Err(e) => {
            ...
            result.insert(JanetKeyword::new("stderr"),
