...
(t/test "filepicker: typing filters items" (fn []
                                            (def w (setup 30 8 :source (fn [] @["README.md" "Cargo.toml" "src/main.rs"])))
  # Type "main" → should filter to src/main.rs
                                            (each ch ["m" "a" "i" "n"]
                                              ((w :handle) w {:type :key :key ch :alt false :ctrl false :shift false}))
                                            (t/assert= ((w :state) :query) "main")
                                            (t/assert= (length ((w :state) :filtered)) 1)
                                            (t/assert= (get ((w :state) :filtered) 0) "src/main.rs")))

(t/test "filepicker: escape clears filter" (fn []
                                            (def w (setup 30 8 :source (fn [] @["README.md" "Cargo.toml" "src/main.rs"])))
                                            ((w :handle) w {:type :key :key "m" :alt false :ctrl false :shift false})
                                            (t/assert= ((w :state) :query) "m")
                                            ((w :handle) w {:type :key :key :escape :alt false :ctrl false :shift false})
                                            (t/assert= ((w :state) :query) "")
                                            (t/assert= (length ((w :state) :filtered)) 3)))

(t/test "filepicker: backspace removes last char from query" (fn []
                                                              (def w (setup 30 8 :source (fn [] @["README.md" "Cargo.toml" "src/main.rs"])))
                                                              ((w :handle) w {:type :key :key "m" :alt false :ctrl false :shift false})
                                                              ((w :handle) w {:type :key :key "a" :alt false :ctrl false :shift false})
                                                              (t/assert= ((w :state) :query) "ma")
                                                              ((w :handle) w {:type :key :key :backspace :alt false :ctrl false :shift false})
                                                              (t/assert= ((w :state) :query) "m")))

(t/test "filepicker: filter resets selection to 0" (fn []
                                                    (def w (setup 30 8 :source (fn [] @["a.txt" "b.txt" "c.txt"])))
  # Move selection down
                                                    ((w :handle) w {:type :key :key :down :alt false :ctrl false :shift false})
                                                    (t/assert= ((w :state) :selected) 1)
  # Type to filter → selection resets
                                                    ((w :handle) w {:type :key :key "a" :alt false :ctrl false :shift false})
                                                    (t/assert= ((w :state) :selected) 0)))

(t/test "filepicker: enter works on filtered list" (fn []
                                                    (var received nil)
                                                    (def w (setup 30 8
                                                            :source (fn [] @["README.md" "Cargo.toml" "src/main.rs"])
                                                            :on-enter (fn [item] (set received item))))
  # Filter to just main.rs
                                                    (each ch ["m" "a" "i" "n"]
                                                      ((w :handle) w {:type :key :key ch :alt false :ctrl false :shift false}))
                                                    ((w :handle) w {:type :key :key :enter :alt false :ctrl false :shift false})
                                                    (t/assert= received "src/main.rs")))

(t/test "filepicker: title shows query when filtering" (fn []
                                                        (def w (setup 30 8 :source (fn [] @["a.txt" "b.txt"])))
                                                        ((w :handle) w {:type :key :key "a" :alt false :ctrl false :shift false})
                                                        (def rows (render-fp w))
                                                        (t/assert-truthy (any-row-contains? rows "│ a"))))

(t/test "filepicker: no match shows empty list" (fn []
                                                 (def w (setup 30 8 :source (fn [] @["a.txt" "b.txt"])))
                                                 (each ch ["z" "z" "z"]
                                                   ((w :handle) w {:type :key :key ch :alt false :ctrl false :shift false}))
                                                 (t/assert= (length ((w :state) :filtered)) 0)))
