...
(defn- render []
  ...
  (def lines (get-lines))
  (def has-prompt (> (length prompt-text) 0))
  (def prompt-len (if has-prompt (+ (length prompt-text) 2) 1))
  ...
  (for i 0 height
    ...
    (if (= i 0)
      (term/write (if has-prompt
                    (string (ui/color :user-label) " " prompt-text (ui/color :reset) " ")
                    " "))
      (term/write pad))
